...
class BaseArrayEditorWidget(QWidget):
    ...
    def _init_handler(self, data: np.ndarray | BaseArrayHandler):
        if isinstance(data, np.ndarray):
            ...
            raise TypeError(
                f"Given data must be of type np.ndarray or BaseArrayHandler, not {type(data)}"
            )
...
class MaskedArrayEditorWidget(BaseArrayEditorWidget):
    ...
    def _init_handler(self, data: np.ma.MaskedArray | MaskedArrayHandler):
        if isinstance(data, np.ma.MaskedArray):
            ...
            raise TypeError(
                f"Given data must be of type np.ma.MaskedArray or MaskedArrayHandler, not {type(data)}"
            )
